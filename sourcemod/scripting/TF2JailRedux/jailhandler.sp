 ...
 *	Reminder that random lr grabs a random int from 1 to LRMAX, having breaks or skips within the enum will result in nothing happening the following round 
*/
...
public void LRMapStartVariables()
{
	...
	{	// For lazy people like me who don't want to increase the maximum for each new LR
		arrLRS[i] = 0;
	}
}
/** 
...
public void AddLRToMenu(Menu & menu)
{
	...
	for (int i = 0; i < sizeof(strLRNames); i++)
	{
		iMax = LR_DEFAULT;	// 5
...
public void ManageRoundStart()
{
	switch (gamemode.iLRType)
	{
		...
		case RandomKill:
		{
			...
			SetPawnTimer(RandSniper, GetRandomFloat(30.0, 60.0), iTimer);
		}
		case Warday:
		{
			gamemode.bIsWarday = true;
...
public void ManageFFTimer()
{
	switch (gamemode.iLRType)
	{
		case HHHDay, TinyRound:
		{
			int iTimer = gamemode.iRoundCount;
			SetPawnTimer(EnableFFTimer, 10.0, iTimer);
		}
		default: {	}
	}
}
...
{
	if (MapHasMusic())	{ song = ""; time = -1.0; }	// I doubt this works, and I don't know of any JB maps with background music
	else
	{
		switch (gamemode.iLRType)
		{
...
stock void _RespawnPlayer(const int userid)
{
	int client = GetClientOfUserId(userid);
	if (!IsClientValid(client))
		return;
	JailFighter(client).Respawn();
	//player.StripToMelee();
}
